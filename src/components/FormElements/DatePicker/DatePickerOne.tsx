...
const DatePickerOne: React.FC<DatePickerOneProps> = ({ date, setDate, className }) => {
  const inputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (inputRef.current) {
      // Initialize Flatpickr on the specific input element
  ...
  return (
    <div>
      <div className="relative">
        <input
          ref={inputRef} // Use the ref to target this input specifically
          className={`form-datepicker w-full rounded-[7px] border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal outline-none transition focus:border-primary active:border-primary dark:border-dark-3 dark:bg-dark-2 dark:focus:border-primary ${className}`}
          placeholder="dd/mm/yyyy"
          value={date}
          readOnly // Prevent manual typing to avoid conflict with Flatpickr
        />
