...
const Advertisements = ({ bannerGroup }) => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-4xl p-8 space-y-6 bg-white rounded-lg shadow-md">
        ...
        <nav aria-label="breadcrumb">
          <ul className="flex space-x-2 text-gray-600">
            <li className="flex items-center">
              <a href="/manage-2x1-banners" className="hover:underline">Advertisements</a>
              <svg className="w-4 h-4 mx-2 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                <path d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" />
              </svg>
            </li>
            <li className="flex items-center">
              <a href="/secondary-banners" className="hover:underline">Secondary Banners</a>
              <svg className="w-4 h-4 mx-2 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                <path d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" />
              </svg>
            </li>
        ...
        <h1 className="text-4xl font-bold text-center mb-8">Advertisements</h1>
...
const CreateBannerModal = ({ setIsModalOpen, setBanners, bannerToEdit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let response;
      if (bannerToEdit) {
        response = await axios.put(`/api/banners2xn/create/${bannerToEdit.id}`, { name: name, layout_type: 'advertisements' });
      } else {
        response = await axios.post('/api/banners2xn/create', { name: name, order: -1, layout_type: 'advertisements' });
      }
      ...
      for (const image of images) {
        if (image.image) {
          ...
          formData.append('layout_type', 'advertisements-' + bannerGroup.id);
