...
export async function GET(req: Request) {
    try {
        ...
        const response = await axios.get('https://ext.web.wamia.tn/rest/V1/mobilemaster/walkthrough', {
            headers: {
...
export async function POST(req: Request) {
    try {
        ...
        const response = await axios.post('https://ext.web.wamia.tn/rest/V1/mobilemaster/walkthrough', requestBody, {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${accessToken}`,
