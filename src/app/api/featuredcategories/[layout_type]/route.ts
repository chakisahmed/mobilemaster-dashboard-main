...
export async function GET(req: Request) {
    try {
        ...
        const response = await axios.get(`https://www.wamia.tn/rest/V1/mobilemaster/featuredcategories/${layoutType}`, {
            headers: {
...
export async function DELETE(req: Request) { 
    try {
        ...
        const url = new URL(req.url);
        const layoutType = url.pathname.split('/').pop();
        const response = await axios.delete(`https://www.wamia.tn/rest/V1/mobilemaster/featuredcategories/${layoutType}`, {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${accessToken}`,
            },
