...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  ...
  const { id } = params;
  try {
    const response = await axios.get(`https://www.wamia.tn/rest/V1/mobilemaster/banner2xn/${id}`, {
      headers: {
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  ...
  const { id } = params;
  try {
    const response = await axios.delete(`https://www.wamia.tn/rest/V1/mobilemaster/banner2xn/${id}`, {
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${accessToken}`,
      },
...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    const response = await axios.put(
      `https://www.wamia.tn/rest/V1/mobilemaster/banner2xn/${id}`,
      { name, sort_order },
      {
        headers: {
          'Content-Type': 'application/json',
